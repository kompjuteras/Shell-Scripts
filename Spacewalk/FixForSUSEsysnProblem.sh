...
# Fix if you use Spacewalk to sync SLES packages. Without this you will packages
# will not sync because of some problem with the "repomd.xml" position in url
# Backup file
cd /usr/lib/python2.7/site-packages/urlgrabber/
mv mirror.py mirror_py_backup-$(date +%Y-%m%d)

# Rewrite file (it's original file + some changes)
cat > /usr/lib/python2.7/site-packages/urlgrabber/mirror.py <<'EOF'
